...
다시 Vector 예제에서 범위를 벗어난 요소에 접근하려고 할 때는 어떻게 해야 할까?<br>
Vector를 작성한 사람은 사용자가 이런 상황에서 무엇을 하길 원하는지 알 수 없다. (대개 Vector 작성자는 이 타입이 어떤 프로그램에서 사용될지조차 모른다.)<br>
Vector를 사용하는 사람은 문제를 일관되게 감지할 수 없다. (만약 감지할 수 있었다면, 애초에 범위를 벗어난 접근이 발생하지 않았을 것이다.)<br>
해결책은 다음과 같다:<br>
Vector 구현자가 범위 밖 접근 시도를 감지하고,<br>
사용자에게 그 사실을 알리는 것이다.<br>
그러면 사용자는 적절한 조치를 취할 수 있다.<br>
예를 들어, Vector::operator[]()는 범위를 벗어난 접근을 감지하고 out_of_range 예외를 던질 수 있다:<br>
```cpp
...
```
throw 문은 Vector::operator[]()를 직접 또는 간접적으로 호출한 함수 중에서 out_of_range 타입의 예외를 처리할 수 있는 핸들러로 제어를 넘깁니다. 이를 위해 구현체는 호출자 함수의 문맥으로 돌아가기 위해 필요한 만큼 함수 호출 스택을 되감기(unwind) 합니다 (§13.5.1). 예를 들면 다음과 같습니다:
이를 위해 구현은 함수 호출 스택을 되감기(unwind)하여 호출자의 컨텍스트로 돌아간다 (§13.5.1).
예:
```cpp
void f(Vector& v)
{
    // ...
    try { // 아래에서 발생하는 예외를 처리
        v[v.size()] = 7; // 범위를 벗어난 접근 시도
    }
    catch (out_of_range) { // 예외 발생: 범위 초과 오류
        // ... 범위 오류 처리 ...
    }
    // ...
}
```
예외를 처리하고 싶은 코드 블록은 try 블록에 넣는다. v[v.size()] = 7은 실패하게 되고, 따라서 catch 절에서 정의된 out_of_range 핸들러가 실행된다. out_of_range 타입은 표준 라이브러리에 정의되어 있으며, 실제로 일부 표준 컨테이너의 접근 함수에서도 사용된다. 예외 처리 메커니즘을 사용하면 오류 처리를 더 간단하고 체계적이며 가독성 있게 만들 수 있다.<br>
<br>

불변 조건 (Invariants): 클래스에 대해 참이라고 가정하는 내용을 클래스 불변 조건(class invariant) 또는 간단히 불변 조건(invariant)이라 부른다.<br>
<br>
불변 조건(invariant)은 다음과 같은 이유로 중요하다:<br>
우리가 정확히 무엇을 원하는지를 이해하는 데 도움을 준다.<br>
구체적으로 명시하도록 강제함으로써, 디버깅과 테스트 이후에 코드가 올바르게 동작할 가능성을 높여준다.<br>
