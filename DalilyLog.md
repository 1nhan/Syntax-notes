...
[상속구조]
>근로자(Employee) <- 정규직(PermanentWorker) <- 영업직(SalesWorker)
>근로자(Employee) <- 임시직(TemporaryWorker)
------------
[임금형태]
>근로자(Employee)			Base Class
>정규직(PermanentWorker)	기본급여
>임시직(TemporaryWorker)	시간급여x일한시간
>영업직(Sales)			기본급여x인센티브
------------
